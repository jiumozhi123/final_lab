...
dd if=/dev/zero of=Input/1G.img bs=1M count=1024（在Input文件夹下建立用二进制数字0填充的1G大小文件，这个过程需要几分钟。注意文件名绝对不能错）

您也可以用您的文件来代替，但必须保证该文件在文件夹Input下，并且文件名为1G.img

建立好文件夹和1G大小文件后，cd final-lab进入final_lab文件夹
...
传送结束后，server端和client端会退出。如果中间遇到问题，它们也会退出并且在shell端打印错误报告。最后，文件传送所用的时间会记录在dbtime.time文件中