...
打开一个shell，用命令找到文件夹final_lab，并cd进入该文件夹
...
您也可以用您的文件来代替，大小随意，但必须保证该文件在文件夹Input下，并且文件名为1G.img
...
再开一个shell，执行./client 127.0.0.1

作用：负责不断接收文件流并写入流

此时socket成功连接，程序会运行几分钟，读写流和发送接收流占用绝大部分时间

传送结束后，server端和client端会退出，client端在退出前会打印传输时间。如果中间遇到问题，它们也会退出并打印错误报告

接收到的文件被保存在Output文件夹下

5、执行./md5

作用：打印输入文件和输出文件的md5值并进行比较，如果相等，就认为传输正确。反之亦然
