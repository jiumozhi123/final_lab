...
每次传输文件前，还需要配置好文件名。直接进入文件夹final_lab-master，其中有个common.h文件，里面有2个宏定义Input和Output，分别定义待传送的文件路径和传送文件的目标路径。每次传送不同的文件时，都需要用gedit打开并修改这两个宏定义中最后的文件名。知道了这个原理，其实路径也是可以随意设定的
